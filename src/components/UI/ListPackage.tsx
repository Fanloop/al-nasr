...
function ListPackage(props: propList) {
  return (
    <div className="shadow-lg rounded-xl p-3 w-full">
      ...
      <div className="flex flex-col gap-4 md:gap-5">
        <div>
          <div className="font-bold tracking-wide mt-5 text-black/75 text-base md:text-sm lg:text-base">
            {props.title}
          </div>
          <div className="font-bold text-black/75 mt-2 md:mt-1 text-base md:text-lg">
            $ {props.price}/<span className="text-cyan-3">person</span>
          </div>
        </div>
        <div className="flex justify-center md:justify-between lg:justify-start gap-4 md:gap-0 lg:gap-4">
          <img src={flight} alt="flight icon" />
          <img src={ticket} alt="ticket icon" />
          <img src={hostel} alt="hostel icon" />
          <img src={visas} alt="Visas icon" />
        </div>
        <button className="block capitalize font-nunito w-full lg:w-fit border-yellow text-yellow border-2 py-2 px-7 rounded-lg hover:bg-yellow hover:text-white transition-all">
          view details
        </button>
      </div>
    </div>
